...
/* =========================================================================
   LEGACY (tạm) — bộ component "gradient" của đợt trước, CHỈ để WorkspacePage cũ
   còn biên dịch trong lúc migrate sang hệ Refined.
   XOÁ file này ở bước 4 khi dựng lại màn Workspace theo design handoff.
   ========================================================================= */
...
export function Card({ className = '', children }: { className?: string; children: ReactNode }) {
  return <div className={`bg-white rounded-[18px] shadow-[0_8px_24px_-10px_rgba(34,41,47,0.12)] ${className}`}>{children}</div>;
}
...
export function GradientStat({ icon, gradient, value, label }: { icon?: ReactNode; gradient: GradKey; value: ReactNode; label: string }) {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0" style={{ background: 'radial-gradient(120% 120% at 100% 0%, rgba(255,255,255,.25), transparent 55%)' }} />
      <div className="relative z-10 flex items-center justify-between">
        {icon ? (
          <span className="flex h-11 w-11 items-center justify-center rounded-[13px] border border-white/25 bg-white/20 text-xl text-white backdrop-blur">{icon}</span>
        ) : (
          <span />
        )}
      </div>
...
export function GradAvatar({ i, children, size = 36 }: { i: number; children: ReactNode; size?: number }) {
  return (
    <span
      ...
      style={{ width: size, height: size, fontSize: size > 40 ? 22 : 13, background: gradOf(i), boxShadow: '0 4px 10px -3px rgba(0,0,0,.25)' }}
    >
      {children}
    </span>
  );
...
export function Pill({ tone = 'muted', children }: { tone?: keyof typeof PILL_TONE; children: ReactNode }) {
  return <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12px] font-medium ${PILL_TONE[tone]}`}>{children}</span>;
}
...
export function Btn({
  ...
  return (
    <button type="button" onClick={onClick} className={`${base} ${styles[variant]} ${className}`} style={variant === 'primary' ? { background: GRADIENTS.orange } : undefined}>
      {children}
    </button>
  );
}
      ...
      <div className="flex gap-1 border-b border-[#f1eff5] px-1">
        {items.map((t) => (
          <button
            ...
            className={`relative px-3 py-2.5 text-[13px] font-medium transition-colors ${active === t.key ? 'text-[#eb5324]' : 'text-[#8b899a] hover:text-[#5e5873]'}`}
          >
            {t.label}
