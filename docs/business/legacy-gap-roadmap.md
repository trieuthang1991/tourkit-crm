...
- ✅ **Tour lẻ/FIT (`SingleTour`)** — nghiệp vụ chốt (uỷ quyền chuyên gia): FIT = CHUYẾN RIÊNG không template, tự chọn ngày. Chuyển đơn FIT thẳng từ báo giá: bỏ trống chuyến + nhập ngày khởi hành → hệ tạo chuyến riêng (`FIT-<mã>`, TotalSlots = đúng số khách → chuyến kín) rồi đặt chỗ như thường; **giá chỗ = giá 3 hạng của báo giá** (SeatPrices override — flow booking mặc định vẫn lấy giá mẫu tour). FIT đặt chốt ngay, không giữ chỗ.
