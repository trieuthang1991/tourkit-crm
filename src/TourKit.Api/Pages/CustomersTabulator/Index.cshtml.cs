...
public class IndexModel : PageModel
{
    ...
    /// <summary>Xuất CSV theo ĐÚNG bộ lọc đang áp (server-side, tối đa 10.000 dòng) — không chỉ trang hiện tại.</summary>
    public async Task<IActionResult> OnGetExportAsync()
    {
        const int max = 10000;
        var result = await _service.ListAsync(1, max, BuildFilter(Request.Query["q"]));
        var typeNames = (await _types.ListAsync()).GroupBy(t => t.Code).ToDictionary(g => g.Key, g => g.First().Name);
        ...
        {
            var typeName = typeNames.TryGetValue(c.CustomerType, out var tn)
                ? tn
                : c.CustomerType.ToString(CultureInfo.InvariantCulture);
