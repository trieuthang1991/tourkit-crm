...
// Phiếu thu CHỜ DUYỆT — biến thể của màn Phiếu thu (Pages/Receipts) với trạng thái mặc định = chờ duyệt.
// Bám bản cũ web/src/features/finance/ReceiptsListPage.tsx: 5 KPI, thanh lọc 9 tiêu chí, cột kép,
// dòng tổng cộng trang, export CSV, modal chi tiết + duyệt/từ chối tại chỗ. DataTables SERVER-SIDE.
// Không có form tạo phiếu: IReceiptService.CreateAsync gắn theo orderId (tạo từ màn đơn).
...
public class IndexModel : TkListPageModel
{
    ...
    /// <summary>Dựng bộ lọc từ query — đúng 9 tiêu chí ReceiptListFilter hỗ trợ.
    /// Khác màn Phiếu thu ở chỗ: thiếu tham số status thì MẶC ĐỊNH lọc trạng thái chờ duyệt.</summary>
    private ReceiptListFilter BuildFilter(string? keyword)
    {
        ...
        return new ReceiptListFilter(
            Q: keyword,
            Status: int.TryParse(q["status"], out var st) ? st : StatusPending,
            From: D("from"), To: D("to"),
    ...
    public async Task<IActionResult> OnGetExportAsync()
    {
        ...
        return File(bytes, "text/csv", "phieu-thu-cho-duyet.csv");
    }
